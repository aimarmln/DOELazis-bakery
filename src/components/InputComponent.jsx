const InputComponent = ({ inputFor, isTextArea }) => {
  ...
  <div className="my-8">
    ...
    {isTextArea ? (
      ...
      <input type="text" name={inputFor.toLowerCase()} id={inputFor.toLowerCase()} className="text-sm w-full py-2 border-b-2 focus:outline-none focus:border-b-darkBrown valid:border-b-darkBrown invalid:border-b-stone-300 placeholder:italic placeholder:text-slate-400 placeholder:text-sm transition-colors" placeholder={`Your ${inputFor}`} autoComplete="off" required />
    )}
  </div>   
  );
}
